...
function openShareModal(id) {
  ...
  <div class="share-modal" id="share-modal">
    <div class="share-modal-header">
      ...
      <div class="share-modal-controls">
        <button class="share-theme-toggle" id="share-theme-btn" onclick="toggleShareTheme()" title="Toggle card theme">
          ${isDark ? '☀️' : '🌙'}
        </button>
    ...
    <div class="share-modal-body">
      ...
      <div class="share-card-wrap">
        <div class="share-card dark" id="share-card" data-theme-card="dark">

          <!-- Decorative bg lines -->
          <svg class="share-card-bg-svg" viewBox="0 0 520 220" preserveAspectRatio="none" aria-hidden="true">
            <polyline points="0,180 65,140 130,160 195,95 260,115 325,60 390,80 455,30 520,50"
              fill="none" stroke="currentColor" stroke-width="1.5" opacity="0.07"/>
            <polyline points="0,200 80,170 160,185 240,145 320,155 400,120 480,135 520,115"
              fill="none" stroke="currentColor" stroke-width="1" opacity="0.04"/>
          </svg>
          <div class="share-card-glow" id="sc-glow"></div>

          <div class="share-card-header">
            <div class="sc-brand">
              <div class="sc-brand-icon">⚡</div>
              <div>
                <div class="sc-brand-name">NxTGen</div>
                <div class="sc-brand-sub">Trading Journal</div>
              </div>
            </div>
            <div class="sc-badge" id="sc-badge"></div>
          </div>

          <div class="sc-hero">
            <div class="sc-hero-left">
              <div class="sc-pair" id="sc-pair">${t.pair}</div>
              <div class="sc-pos-session" id="sc-pos-session">${t.pos} · ${t.kz}</div>
            </div>
            <div class="sc-hero-right">
              <div class="sc-pnl" id="sc-pnl"></div>
              <div class="sc-pnl-label">Net PnL</div>
            </div>
          </div>
          ...
          <div class="sc-stats">
            <div class="sc-stat">
              <span class="sc-stat-label">Risk · Reward</span>
              <span class="sc-stat-val" id="sc-rr">${t.rr}</span>
            </div>
            <div class="sc-stat sc-stat-mid">
              <span class="sc-stat-label">TF Alignment</span>
              <span class="sc-stat-val" id="sc-tf">${t.tf || '—'}</span>
            </div>
            <div class="sc-stat">
              <span class="sc-stat-label">Strategy</span>
              <span class="sc-stat-val" id="sc-strategy">${(t.strategy || '—').split(' - ')[0]}</span>
            </div>
          </div>
          ...
          <div class="sc-tags">
            <div class="sc-tag"><span class="sc-tag-dot sc-tag-dot-gold"></span><span id="sc-tag-kz">${t.kz}</span></div>
            <div class="sc-tag"><span class="sc-tag-dot sc-tag-dot-blue"></span><span id="sc-tag-date">${t.date}</span></div>
            <div class="sc-tag"><span class="sc-tag-dot sc-tag-dot-green"></span>${t.account}</div>
          </div>

          <div class="sc-notes-wrap" id="sc-notes-wrap">
            <div class="sc-notes" id="sc-notes">${t.notes || ''}</div>
          </div>

          <div class="sc-footer-row">
            <div class="sc-stars" id="sc-stars"></div>
            <div class="sc-risk-chip" id="sc-risk-chip">${t.risk || '—'} risk</div>
          </div>

          <div class="sc-footer">
            <span class="sc-footer-url">dabossmira.github.io/NxTGen-Journal</span>
            <span class="sc-footer-brand">⚡ NxTGen</span>
          </div>
        </div>
      </div>

      <!-- Export format picker -->
      <div class="share-format-section">
        <div class="share-format-label">Export format</div>
        <div class="share-format-tabs">
          <button class="share-fmt-btn active" id="fmt-jpg" onclick="setShareFmt('jpg',this)">
            <span class="fmt-icon">🖼</span>
            <span class="fmt-name">JPG Image</span>
            <span class="fmt-desc">Best for sharing on social</span>
          </button>
          <button class="share-fmt-btn" id="fmt-pdf" onclick="setShareFmt('pdf',this)">
            <span class="fmt-icon">📄</span>
            <span class="fmt-name">PDF Document</span>
            <span class="fmt-desc">Best for records &amp; printing</span>
          </button>
        </div>
      </div>
      ...
      <div class="share-actions-row">
        <button class="share-action-btn share-action-download" id="share-dl-btn" onclick="shareDownload(${id})">
          <span class="share-action-spinner" id="share-dl-spinner"></span>
          <span id="share-dl-label">⬇ Download</span>
        </button>
        <button class="share-action-btn share-action-native" id="share-native-btn" onclick="shareNative(${id})">
          <span class="share-action-spinner" id="share-native-spinner"></span>
          <span id="share-native-label">🔗 Share</span>
        </button>
      </div>
    </div>
  </div>`;
  ...
  // Load libraries then populate card
  _ensureShareLibs(() => _populateShareCard(t));
}

function _populateShareCard(t) {
  // Outcome badge
  const badge = document.getElementById('sc-badge');
  const outcomeMap = { Win: ['WIN', 'sc-badge-win'], Loss: ['LOSS', 'sc-badge-loss'], 'B.E': ['B/E', 'sc-badge-be'] };
  const [label, cls] = outcomeMap[t.outcome] || ['—', ''];
  badge.textContent = label;
  badge.className = 'sc-badge ' + cls;

  // Glow color
  const glow = document.getElementById('sc-glow');
  if (t.outcome === 'Win') glow.className = 'share-card-glow glow-green';
  else if (t.outcome === 'Loss') glow.className = 'share-card-glow glow-red';
  else glow.className = 'share-card-glow glow-gold';

  // PnL
  const pnlEl = document.getElementById('sc-pnl');
  const sign = t.pnl > 0 ? '+' : '';
  pnlEl.textContent = sign + t.pnl + '%';
  pnlEl.className = 'sc-pnl ' + (t.pnl > 0 ? 'sc-pnl-win' : t.pnl < 0 ? 'sc-pnl-loss' : 'sc-pnl-be');

  // Stars
  const starsEl = document.getElementById('sc-stars');
  starsEl.innerHTML = [1,2,3,4,5].map(n =>
    `<span style="opacity:${n <= t.rating ? 1 : 0.2}">★</span>`
  ).join('');

  // Notes visibility
  const notesWrap = document.getElementById('sc-notes-wrap');
  const notesEl = document.getElementById('sc-notes');
  if (!t.notes || t.notes.trim() === '') {
    notesWrap.style.display = 'none';
  } else {
    notesEl.textContent = t.notes.length > 120 ? t.notes.substring(0, 117) + '…' : t.notes;
  }
}

let _shareFmt = 'jpg';

function setShareFmt(fmt, btn) {
  _shareFmt = fmt;
  document.querySelectorAll('.share-fmt-btn').forEach(b => b.classList.remove('active'));
...
function toggleShareTheme() {
  ...
  const current = card.getAttribute('data-theme-card');
  if (current === 'dark') {
    card.setAttribute('data-theme-card', 'light');
    card.className = 'share-card light';
    btn.textContent = '🌙';
  } else {
    card.setAttribute('data-theme-card', 'dark');
    card.className = 'share-card dark';
    btn.textContent = '☀️';
  }
}
...
function _setShareLoading(btnId, spinnerId, labelId, loading, labelText) {
...
async function shareDownload(id) {
  const t = trades.find(x => x.id === id);
  if (!t) return;
  _setShareLoading('share-dl-btn', 'share-dl-spinner', 'share-dl-label', true);
  try {
    const canvas = await _captureCard();
    const filename = `NxTGen_${t.pair}_${t.date}`;
    ...
    if (_shareFmt === 'pdf') {
      ...
      pdf.addImage(imgData, 'JPEG', 15, 20, imgW, imgH);
  ...
  _setShareLoading('share-dl-btn', 'share-dl-spinner', 'share-dl-label', false, '⬇ Download');
}

async function shareNative(id) {
  const t = trades.find(x => x.id === id);
  if (!t) return;
  _setShareLoading('share-native-btn', 'share-native-spinner', 'share-native-label', true);
  try {
    const canvas = await _captureCard();
    const mimeType = _shareFmt === 'pdf' ? 'image/jpeg' : 'image/jpeg';
    canvas.toBlob(async (blob) => {
      const file = new File([blob], `NxTGen_${t.pair}_${t.date}.jpg`, { type: 'image/jpeg' });
      const shareData = {
        files: [file],
        title: `NxTGen Trade · ${t.pair} · ${t.outcome}`,
        text: `${t.pair} ${t.pos} · ${t.pnl > 0 ? '+' : ''}${t.pnl}% · ${t.kz} — NxTGen Trading Journal`,
      };
      if (navigator.canShare && navigator.canShare(shareData)) {
        try {
          ...
          if (e.name !== 'AbortError') _copyCardToClipboard(canvas);
        }
      } else {
        _copyCardToClipboard(canvas);
      }
      _setShareLoading('share-native-btn', 'share-native-spinner', 'share-native-label', false, '🔗 Share');
    }, 'image/jpeg', 0.95);
  } catch (e) {
    console.error('Share native error:', e);
    showToast('Share failed — try Download instead', 'danger');
    _setShareLoading('share-native-btn', 'share-native-spinner', 'share-native-label', false, '🔗 Share');
  }
}

function _copyCardToClipboard(canvas) {
  canvas.toBlob(async (blob) => {
    try {
...
function _ensureShareLibs(cb) {
  const loaded = () => typeof html2canvas !== 'undefined' && typeof window.jspdf !== 'undefined';
  if (loaded()) { cb(); return; }
