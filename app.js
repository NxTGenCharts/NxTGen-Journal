...
async function _wlLoad() {
  ...
  _wlData = (data || []).map(r => ({
    ...
    weekLabel: r.week_label,   // "Week of May 12"
    weekDate:  r.week_date,    // "2026-05-12"
    dxy:       r.dxy_bias || 'neu',
...
function _wlRenderWeekContent(week, container) {
  ...
  // Checklist
  const clHtml = PREWEEK_CHECKS.map((item, i) => {
    const checked = week.checklist.includes(i);
    return `<div class="cl-item${checked ? ' checked' : ''}" onclick="_wlToggleCheck('${week.id}',${i})">
  ...
  container.innerHTML = `
    <div class="wl-week-header">
      <div class="wl-week-meta">
        ...
        <div class="wl-week-date">${week.weekDate}</div>
      </div>
    ...
    <div class="wl-checklist-section">
      <div class="sec-head">Pre-Week Checklist</div>
      <div class="wl-checklist-grid">${clHtml}</div>
    </div>
  `;
}
...
function _wlShowWeekModal(week, defaultDate) {
  ...
  document.getElementById('wl-modal-body').innerHTML = `
    <div class="wl-form-row">
      <label class="wl-form-label">Week Start Date</label>
      <input type="date" class="wl-form-input" id="wl-f-date" value="${week ? week.weekDate : defaultDate}">
    </div>
...
async function _wlSaveWeekForm(existingId) {
  const date = document.getElementById('wl-f-date').value;
  ...
  const label = 'Week of ' + dt.toLocaleDateString('en-GB', opts);
  const qKey  = _wlQuarterKey(date);
  ...
  if (existingId) {
    ...
    const week = { id: null, quarter: qKey, weekLabel: label, weekDate: date, dxy, market: mkt, pairs: [], checklist: [] };
    await _wlSaveWeek(week);
    _wlActiveQ      = qKey;
...
function _wlShowPairModal(pair) {
  ...
  document.getElementById('wl-pair-modal-body').innerHTML = `
    ...
    <div class="wl-form-2col">
      ...
      <div class="wl-form-row">
        <label class="wl-form-label">R:R Target</label>
        <input type="text" class="wl-form-input" id="wl-p-rr" value="${pair&&pair.rr||''}" placeholder="1:3">
      </div>
    </div>
    <div class="wl-form-row">
...
async function _wlSavePair() {
  ...
  const rr       = document.getElementById('wl-p-rr').value;
  ...
  const pairData = { name: name.toUpperCase(), priority, bias, rr, note, tfs, charts: [..._wlPendingCharts] };
...
function buildAccounts() {
  // Derive account stats live from the trades array
  const accountNames = _getCustomAccounts();
  const accsHTML = accountNames.map(name => {
    ...
    return `
    <div class="acc-card">
      <div class="acc-card-head"><div class="acc-name">${name}</div><span class="acc-status active">Active</span></div>
      <div class="acc-row"><span class="k">Trades</span><span class="v">${at.length || '—'}</span></div>
      <div class="acc-row"><span class="k">Win Rate</span><span class="v" style="color:${at.length ? 'var(--green)' : 'var(--text3)'}">${at.length ? wr + '%' : '—'}</span></div>
      <div class="acc-row"><span class="k">Net PnL</span><span class="v" style="color:${pnlColor}">${at.length ? (pnl > 0 ? '+' : '') + pnl + '%' : '—'}</span></div>
    </div>`;
  }).join('');
  const grid = document.getElementById('accounts-grid');
  if (grid) grid.innerHTML = accsHTML || '<div style="color:var(--text3);padding:20px;text-align:center">No accounts yet. Add trades to see account stats.</div>';
  const ml = document.getElementById('milestones-list');
  if (ml) ml.innerHTML = MILESTONES.map((m, i) => `<div class="cl-item" onclick="this.classList.toggle('checked');this.querySelector('.cl-box').textContent=this.classList.contains('checked')?'✓':''"><div class="cl-box"></div><span class="cl-text">${m}</span></div>`).join('');
}
