...
// SEED_TRADES removed — every user starts with a clean empty journal
...
// WL_PAIRS: per-user, loaded from Supabase journal_user_prefs
let WL_PAIRS=[{name:"GBPUSD",priority:"🔴 HIGH",bias:"Bear",tfs:[{tf:"Weekly",bias:"bear"},{tf:"Daily",bias:"bear"},{tf:"4H",bias:"bear"},{tf:"1H",bias:"neu"}],note:"Setup: NxtGen IRL>ERL · Key: 1.2650 OB · R:R 1:3 · London kill zone · Watch 1am manipulation"},
...
// MILESTONES: per-user, loaded from Supabase journal_user_prefs
let MILESTONES=["Pass GFT $5k Phase 1","Pass GFT $10k Phase 1","Receive first funded payout","Scale to $50k funded","Scale to $100k funded","Open personal live account","Consistent $1k/month"];
// GOALS: per-user, loaded from Supabase journal_user_prefs
let GOALS=[
...
// AFFIRMATIONS: per-user, loaded from Supabase journal_user_prefs
let AFFIRMATIONS=["I only take A+ setups. I am patient.","I follow my plan. I do not chase price.","My edge works over time. One trade does not define me.","I protect my capital above all else.","I am calm, disciplined, and consistent.","Losses are the cost of doing business. I learn and move on.","I trade the model, not my emotions."];
...
async function loadTrades() {
  ...
  if (data.length === 0) {
    // Brand-new user — empty journal, no seeding
    trades = []; tradeState = {};
    // Reset per-user data to empty so previous user's data never bleeds through
    GOALS = []; WL_PAIRS = []; MILESTONES = []; AFFIRMATIONS = [];
...
/* Brand-new user — start with a completely empty journal */
async function seedDemoTrades() {
  // No-op: seeding removed so every user's journal is isolated.
  // A new user simply starts with zero trades.
  trades = [];
  tradeState = {};
  showToast('Welcome to NxTGen Journal! Add your first trade to get started. 🚀', 'restore');
}
...
function buildWatchlist() {
  ...
  if (!WL_PAIRS || WL_PAIRS.length === 0) {
    cont.innerHTML = `<div class="empty-state-card"><div class="empty-icon">📋</div><div class="empty-title">No watchlist yet</div><div class="empty-sub">Add pairs each Sunday to prepare your weekly top-down analysis.</div></div>`;
  } else {
...
function buildAccounts() {
  // Derive account stats live from actual trade data — no hardcoded numbers
  const grid = document.getElementById('accounts-grid');
  const accountNames = [...new Set(trades.map(t => t.account).filter(Boolean))];

  if (accountNames.length === 0) {
    grid.innerHTML = `<div class="empty-state-card" style="grid-column:span 2">
      <div class="empty-icon">💰</div>
      <div class="empty-title">No account data yet</div>
      <div class="empty-sub">Your account stats will appear here as you log trades.</div>
    </div>`;
  } else {
    grid.innerHTML = accountNames.map(name => {
      ...
      const bes    = at.filter(t => t.outcome === 'B.E').length;
      const wr     = ((wins / at.length) * 100).toFixed(1);
      const pnl    = at.reduce((s, t) => s + t.pnl, 0).toFixed(2);
      const pnlPos = parseFloat(pnl) >= 0;
      return `<div class="acc-card">
        <div class="acc-card-head">
          <div class="acc-name">${name}</div>
          <span class="acc-status active">${name === 'PaperTrading' ? 'Practice' : 'Active'}</span>
        </div>
        <div class="acc-row"><span class="k">Trades</span><span class="v">${at.length}</span></div>
        <div class="acc-row"><span class="k">Win Rate</span><span class="v" style="color:${parseFloat(wr)>=60?'var(--green)':'var(--red)'}">${wr}%</span></div>
        <div class="acc-row"><span class="k">Net PnL</span><span class="v" style="color:${pnlPos?'var(--green)':'var(--red)'}">${pnlPos?'+':''}${pnl}%</span></div>
        <div class="acc-row"><span class="k">W / L / BE</span><span class="v">${wins} / ${losses} / ${bes}</span></div>
      </div>`;
    }).join('');
  }
  ...
  if (ml) {
    if (!MILESTONES || MILESTONES.length === 0) {
      ml.innerHTML = `<div class="empty-state-card"><div class="empty-icon">🏆</div><div class="empty-title">No milestones yet</div><div class="empty-sub">Add trading milestones to track your journey.</div></div>`;
    } else {
      ml.innerHTML = MILESTONES.map(m => `<div class="cl-item" onclick="this.classList.toggle('checked');this.querySelector('.cl-box').textContent=this.classList.contains('checked')?'✓':''"><div class="cl-box"></div><span class="cl-text">${m}</span></div>`).join('');
    }
  }
}
...
function buildGoals() {
  ...
  if (!GOALS || GOALS.length === 0) {
    goalsList.innerHTML = `<div class="empty-state-card"><div class="empty-icon">🎯</div><div class="empty-title">No goals set yet</div><div class="empty-sub">Set quarterly goals to stay focused and track your progress.</div></div>`;
  } else {
  ...
  if (affEl) {
    if (!AFFIRMATIONS || AFFIRMATIONS.length === 0) {
      affEl.innerHTML = `<div class="empty-state-card"><div class="empty-icon">💬</div><div class="empty-title">No affirmations yet</div><div class="empty-sub">Add your trading mantras to stay disciplined and focused.</div></div>`;
    } else {
      affEl.innerHTML = AFFIRMATIONS.map((a, i) => `<div class="rule-card"><div class="rule-num">${String(i + 1).padStart(2, '0')}</div><div class="rule-text" style="font-style:italic">"${a}"</div></div>`).join('');
    }
  }
}
...
function updateKPIs() {
  ...
  // Sync dashboard cover to current quarter dynamically
  const _dn = new Date(), _cy = _dn.getFullYear(), _cm = _dn.getMonth() + 1;
  const _cq = _cm <= 3 ? 1 : _cm <= 6 ? 2 : _cm <= 9 ? 3 : 4;
  const _ql = { 1:'Jan/Feb/Mar', 2:'Apr/May/Jun', 3:'Jul/Aug/Sep', 4:'Oct/Nov/Dec' };
  const _ctEl = document.getElementById('dash-cover-title');
  const _cpEl = document.getElementById('dash-cover-period');
  const _csEl = document.getElementById('dash-cover-sub-text');
  if (_ctEl)  _ctEl.textContent  = `Q${_cq} ${_cy} — ${_ql[_cq]}`;
  if (_cpEl)  _cpEl.textContent  = `QUARTERLY PERFORMANCE · ${_cy}`;
  if (_csEl) {
    if (total === 0) {
      _csEl.textContent = 'No trades yet — log your first trade to see your stats!';
    } else {
      const _qt = trades.filter(t => { const ty=parseInt(t.date.slice(0,4)),tm=parseInt(t.date.slice(5,7)),tq=tm<=3?1:tm<=6?2:tm<=9?3:4; return ty===_cy&&tq===_cq; });
      const _qw = _qt.filter(t=>t.outcome==='Win').length;
      const _qr = _qt.length ? ((_qw/_qt.length)*100).toFixed(1) : 0;
      const _qp = _qt.reduce((a,t)=>a+t.pnl,0).toFixed(1);
      _csEl.textContent = `${_qt.length} trades · Win rate ${_qr}% · Net PnL ${_qp>0?'+':''}${_qp}%`;
    }
  }
}
...
function buildSidebarYears() {
  ...
  // Only show the current year + any year that has actual trades
  const yearsWithTrades = [...new Set(trades.map(t => getYear(t.date)))];
  const years = [...new Set([curYear, ...yearsWithTrades])].sort((a, b) => b - a);
  ...
  cont.innerHTML = years.map(year => {
    return `<div class="sb-section">
      ...
      <div id="yr-quarters-${year}">${[1, 2, 3, 4].filter(q => {
        const qt = trades.filter(t => getYear(t.date) === year && getQuarter(t.date) === q);
        return qt.length > 0 || (year === curYear && q === curQ);
      }).map(q => {
        const qt = trades.filter(t => getYear(t.date) === year && getQuarter(t.date) === q);
        const isActive = year === curYear && q === curQ;
        const hasTrades = qt.length > 0;
        return `<div class="sb-item${isActive ? ' q2' : ''}" id="sb-q-${year}-${q}" onclick="openQuarter(${year},${q},this)">
...
function renderQuarterPage(year, q) {
  ...
  document.getElementById('quarter-page-inner').innerHTML = `
    ...
    <div class="q-empty-state">
      ...
      <div class="q-empty-title">No trades yet for Q${q} ${year}</div>
      <div class="q-empty-sub">This quarter covers ${Q_MONTHS[q]}. Once you log trades, your performance stats, month breakdown, and full trade history will appear here.</div>
      <button class="q-empty-btn" onclick="openModal()">＋ Log Your First Trade</button>
    </div>` : `
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  await loadUserPrefs();   // per-user: goals, watchlist, affirmations, milestones
...
// ══════════════════════════════════════════════════════════════════════
// PER-USER PREFS — goals, watchlist, affirmations, milestones
// Stored in Supabase: journal_user_prefs (user_id TEXT, key TEXT, value JSONB)
...
async function loadUserPrefs() {
  ...
  try {
    const { data, error } = await sb
      .from('journal_user_prefs')
      .select('key, value')
      .eq('user_id', _currentUser.id);

    if (error) {
      // Table may not exist yet — silently continue with current values
      if (!error.message.includes('does not exist')) {
        console.warn('loadUserPrefs:', error.message);
      }
      return;
    }

    if (!data || data.length === 0) return; // New user — keep defaults

    data.forEach(row => {
      try {
        const val = typeof row.value === 'string' ? JSON.parse(row.value) : row.value;
        if (row.key === 'goals')        GOALS        = val;
        if (row.key === 'wl_pairs')     WL_PAIRS     = val;
        if (row.key === 'affirmations') AFFIRMATIONS = val;
        if (row.key === 'milestones')   MILESTONES   = val;
      } catch(e) { console.warn('loadUserPrefs parse error:', row.key, e); }
    });
  } catch(e) { console.warn('loadUserPrefs exception:', e); }
}

async function _saveUserPref(key, value) {
  if (!_currentUser) return;
  try {
    await sb.from('journal_user_prefs').upsert(
      { user_id: _currentUser.id, key, value: JSON.stringify(value) },
      { onConflict: 'user_id,key' }
    );
  } catch(e) { console.warn('_saveUserPref error:', key, e); }
}

// Call these after mutating the global arrays
async function persistGoals()        { await _saveUserPref('goals',        GOALS);        buildGoals(); }
async function persistWatchlist()    { await _saveUserPref('wl_pairs',     WL_PAIRS);     buildWatchlist(); }
async function persistAffirmations() { await _saveUserPref('affirmations', AFFIRMATIONS); buildGoals(); }
async function persistMilestones()   { await _saveUserPref('milestones',   MILESTONES);   buildAccounts(); }
