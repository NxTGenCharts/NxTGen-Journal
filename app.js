...
async function loadTrades() {
  ...
  if (data.length === 0) {
    // Brand new user — start completely empty, no seeding
...
/* New users start with a completely clean journal — no demo data */
async function seedDemoTrades() {
  trades = [];
  tradeState = {};
  // Reset per-user collections so nothing from another session bleeds through
  GOALS = []; WL_PAIRS = []; MILESTONES = []; AFFIRMATIONS = [];
  showToast('Welcome to NxTGen Journal! Log your first trade to get started. 🚀', 'restore');
}
...
function buildAccounts() {
  ...
  // Get all unique account names that appear in actual trade data
  ...
  if (accountNames.length === 0) {
    grid.innerHTML = `<div class="empty-state-card" style="grid-column:span 2">
      ...
      <div class="empty-sub">Your account stats will appear here as you log trades. Each account you use will get its own card automatically.</div>
    </div>`;
  } else {
    grid.innerHTML = accountNames.map(name => {
      ...
      const wr     = at.length ? ((wins / at.length) * 100).toFixed(1) : '0.0';
      const pnl    = at.reduce((s, t) => s + t.pnl, 0).toFixed(2);
      const pnlPos = parseFloat(pnl) >= 0;
      const isPaper = name === 'PaperTrading';
      return `<div class="acc-card">
        <div class="acc-card-head">
          <div class="acc-name">${name}</div>
          <span class="acc-status active">${isPaper ? 'Practice' : 'Active'}</span>
        </div>
    ...
    }).join('') || `<div class="empty-state-card" style="grid-column:span 2">
      <div class="empty-icon">💰</div>
      <div class="empty-title">No account data yet</div>
      <div class="empty-sub">Log trades with an account assigned to see stats here.</div>
    </div>`;
  }
  ...
  if (ml) {
    if (!MILESTONES || MILESTONES.length === 0) {
      ml.innerHTML = `<div class="empty-state-card"><div class="empty-icon">🏆</div><div class="empty-title">No milestones set</div><div class="empty-sub">Add milestones to track your journey.</div></div>`;
    } else {
...
function buildGoals() {
  ...
  if (affEl) {
    if (!AFFIRMATIONS || AFFIRMATIONS.length === 0) {
      affEl.innerHTML = `<div class="empty-state-card"><div class="empty-icon">💬</div><div class="empty-title">No affirmations yet</div><div class="empty-sub">Add trading mantras to stay disciplined and focused.</div></div>`;
    } else {
...
function updateKPIs() {
  ...
  // Sync dashboard cover to current quarter — never hardcoded
  (function() {
    const _d = new Date(), _y = _d.getFullYear(), _m = _d.getMonth() + 1;
    const _q = _m <= 3 ? 1 : _m <= 6 ? 2 : _m <= 9 ? 3 : 4;
    ...
    const subTxtEl = document.getElementById('dash-cover-sub-text');
    if (titleEl)  titleEl.textContent  = 'Q' + _q + ' ' + _y + ' — ' + _ql[_q];
    if (periodEl) periodEl.textContent = 'QUARTERLY PERFORMANCE · ' + _y;
    if (subTxtEl) {
      if (total === 0) {
        subTxtEl.textContent = 'No trades yet — log your first trade to see your stats!';
      } else {
        const _qt = trades.filter(t => {
          const ty = parseInt(t.date.slice(0,4)), tm = parseInt(t.date.slice(5,7));
          const tq = tm<=3?1:tm<=6?2:tm<=9?3:4;
          return ty === _y && tq === _q;
        });
        const _qw = _qt.filter(t => t.outcome === 'Win').length;
        const _qr = _qt.length ? ((_qw/_qt.length)*100).toFixed(1) : '0.0';
        const _qp = _qt.reduce((a,t) => a+t.pnl, 0).toFixed(1);
        subTxtEl.textContent = _qt.length + ' trades · Win rate ' + _qr + '% · Net PnL ' + (parseFloat(_qp)>0?'+':'') + _qp + '%';
      }
    }
  })();
}
...
function buildSidebarYears() {
  ...
  // Always include current year; also include any year that has actual trades
...
function renderQuarterPage(year, q) {
  ...
  document.getElementById('quarter-page-inner').innerHTML = `
    ...
    <div class="q-empty-state">
      ...
      <div class="q-empty-sub">This quarter covers ${Q_MONTHS[q]}. Once you log trades, your stats and full history will appear here.</div>
...
// Table: journal_user_prefs (user_id TEXT, key TEXT, value JSONB)
// Unique constraint on (user_id, key) for upsert
// ══════════════════════════════════════════════════════════════════════
...
async function loadUserPrefs() {
  ...
  try {
    ...
    if (!data || data.length === 0) return; // New user — keep empty defaults
...
// Call these after mutating the global arrays to persist changes
