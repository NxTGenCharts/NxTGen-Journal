...
// Cache for themed logo PNG data URLs: { dark, light }
const _smLogoCache = {};

/**
 * Renders logo.svg onto an offscreen canvas with a solid colour fill applied,
 * then returns a PNG data URL. This guarantees html2canvas always gets a
 * rasterised image with correct colours — no CSS filter dependency.
 *
 * @param {'dark'|'light'} theme - card theme
 * @param {number} w - output width in px
 * @param {number} h - output height in px
 */
async function _smGetThemedLogo(theme, w, h) {
  const key = `${theme}-${w}-${h}`;
  if (_smLogoCache[key]) return _smLogoCache[key];

  try {
    // 1. Load the SVG as text
    const res = await fetch('logo.svg');
    let svgText = await res.text();

    // 2. Inject a CSS rule that forces every element to the target colour.
    //    dark card → white (#ffffff), light card → near-black (#0f172a)
    const colour = theme === 'light' ? '#0f172a' : '#ffffff';

    // Insert a <style> that overrides fills and strokes to our colour
    // and also forces image elements to use a CSS filter for recolouring
    const styleInject = `<style>
      * { color: ${colour} !important; fill: ${colour} !important; stroke: none !important; }
      image { filter: brightness(0) ${theme === 'light' ? '' : 'invert(1)'} !important; }
      rect.s0 { fill: transparent !important; }
    </style>`;

    svgText = svgText.replace(/<svg([^>]*)>/, `<svg$1>${styleInject}`);

    // 3. Convert to blob URL
    const blob    = new Blob([svgText], { type: 'image/svg+xml' });
    const blobUrl = URL.createObjectURL(blob);

    // 4. Draw onto an offscreen canvas at the requested size
    const result = await new Promise((resolve, reject) => {
      ...
      img.onload   = () => {
        ...
        _smLogoCache[key] = dataUrl;
        resolve(dataUrl);
      };
      img.onerror  = () => { URL.revokeObjectURL(blobUrl); reject(new Error('Logo render failed')); };
      img.src      = blobUrl;
    });

    return result;
  } catch (e) {
    console.warn('Logo theme render failed:', e);
    return null;
  }
}
...
async function _smCapture() {
  ...
  // Gather all logo images with their natural rendered sizes
  const logoImgs   = Array.from(card.querySelectorAll('img[src*="logo.svg"]'));
  const origSrcs   = logoImgs.map(img => img.getAttribute('src'));
  const origStyles = logoImgs.map(img => img.style.cssText);

  // Pre-render a coloured PNG for each unique size
  const replacements = await Promise.all(logoImgs.map(async img => {
    const rect = img.getBoundingClientRect();
    const w    = Math.round(rect.width  * (window.devicePixelRatio || 1)) * 5 || 480;
    const h    = Math.round(rect.height * (window.devicePixelRatio || 1)) * 5 || 120;
    const dataUrl = await _smGetThemedLogo(theme, w, h);
    return dataUrl;
  }));

  // Swap src and strip CSS filter (html2canvas ignores filter anyway)
  logoImgs.forEach((img, i) => {
    if (replacements[i]) {
      img.setAttribute('src', replacements[i]);
      img.style.filter = 'none';
      img.style.opacity = i === 0 ? '1' : '0.55';
    }
  });

  // Wait two frames for browser to paint the new images
  ...
  const canvas = await html2canvas(card, {
    ...
    imageTimeout: 5000,
    onclone: (doc) => {
      // Ensure filters are stripped in the cloned doc too
