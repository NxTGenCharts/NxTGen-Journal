...
// SEED_TRADES removed — new users always start with an empty journal
const SEED_TRADES = [];];
...
async function loadTrades() {
  ...
  if (data.length === 0) {
    // Brand-new user — zero trades, zero personal data
    trades = []; tradeState = {};
    WL_PAIRS = []; GOALS = []; MILESTONES = []; AFFIRMATIONS = [];
...
/* New users get a completely clean empty journal — no demo data */
...
function buildWatchlist() {
  ...
  if (!WL_PAIRS || WL_PAIRS.length === 0) {
    cont.innerHTML = `<div class="empty-state-card"><div class="empty-icon">📋</div><div class="empty-title">No watchlist yet</div><div class="empty-sub">Add pairs each Sunday to prepare your weekly analysis.</div></div>`;
  } else {
...
function buildAccounts() {
  ...
  if (accountNames.length === 0) {
    grid.innerHTML = `<div class="empty-state-card" style="grid-column:span 2">
      ...
      <div class="empty-sub">Your account stats will appear here automatically as you log trades.</div>
    </div>`;
  } else {
  ...
  if (ml) {
    if (!MILESTONES || MILESTONES.length === 0) {
      ml.innerHTML = `<div class="empty-state-card"><div class="empty-icon">🏆</div><div class="empty-title">No milestones set</div><div class="empty-sub">Add milestones to track your progress journey.</div></div>`;
    } else {
...
function buildGoals() {
  ...
  if (!GOALS || GOALS.length === 0) {
    gl.innerHTML = `<div class="empty-state-card"><div class="empty-icon">🎯</div><div class="empty-title">No goals set yet</div><div class="empty-sub">Set quarterly goals to stay focused and track your progress.</div></div>`;
  } else {
  ...
  if (affEl) {
    if (!AFFIRMATIONS || AFFIRMATIONS.length === 0) {
      affEl.innerHTML = `<div class="empty-state-card"><div class="empty-icon">💬</div><div class="empty-title">No affirmations yet</div><div class="empty-sub">Add your trading mantras to stay disciplined.</div></div>`;
    } else {
...
function updateKPIs() {
  ...
  // Sync cover to current quarter — never hardcoded
  (function(){
    ...
    const te=document.getElementById('dash-cover-title');
    const pe=document.getElementById('dash-cover-period');
    const se=document.getElementById('dash-cover-sub-text');
    if(te) te.textContent='Q'+_q+' '+_y+' — '+_ql[_q];
    if(pe) pe.textContent='QUARTERLY PERFORMANCE · '+_y;
    if(se){
      if(total===0){ se.textContent='No trades yet — log your first trade to see your stats!'; }
      else {
        const _qt=trades.filter(t=>{const ty=parseInt(t.date.slice(0,4)),tm=parseInt(t.date.slice(5,7)),tq=tm<=3?1:tm<=6?2:tm<=9?3:4;return ty===_y&&tq===_q;});
        ...
        se.textContent=_qt.length+' trades · Win rate '+_qr+'% · Net PnL '+(parseFloat(_qp)>=0?'+':'')+_qp+'%';
      }
    }
  })();
}
...
function buildSidebarYears() {
  ...
  // Show current year always; only include past years if they have trades
  ...
  cont.innerHTML = years.map(year => {
    return `<div class="sb-section">
      ...
      <div id="yr-quarters-${year}">${[1, 2, 3, 4].filter(q => {
        // Only show quarters that have trades OR is the current active quarter
...
function renderQuarterPage(year, q) {
  ...
  document.getElementById('quarter-page-inner').innerHTML = `
    ...
    <div class="q-empty-state">
      ...
      <div class="q-empty-sub">This quarter covers ${Q_MONTHS[q]}. Once you log trades, your stats, month breakdown and history will appear here.</div>
...
// Supabase table: journal_user_prefs  (user_id TEXT, key TEXT, value JSONB)
// Unique constraint on (user_id, key) enables safe upsert
// ══════════════════════════════════════════════════════════════════════
...
async function loadUserPrefs() {
  ...
  try {
    ...
    if (error) {
      ...
      if (!error.message.includes('does not exist')) console.warn('loadUserPrefs:', error.message);
      return;
    }
    if (!data || data.length === 0) return; // New user — stays with empty defaults
    ...
    data.forEach(row => {
      ...
      } catch(e) { console.warn('loadUserPrefs parse:', row.key, e); }
    });
...
async function _saveUserPref(key, value) {
  ...
  } catch(e) { console.warn('_saveUserPref:', key, e); }
}
