...
function _renderDetail(id) {
  ...
  const viewStats = `
    <div class="detail-stats">
      ...
      <div class="stat-row"><span class="stat-label">TF Alignment</span><span class="stat-val">${t.tf || '—'}</span></div>
  ...
  const editForm = `
    <div class="form-grid" style="margin-bottom:14px">
      ...
      <div class="form-field"><label class="form-label">Account</label><select class="form-select" id="e-acc" onchange="_handleCustomSelect(this,'e-acc-custom')"><option${t.account === 'PaperTrading' ? ' selected' : ''}>PaperTrading</option><option${t.account === 'GFT $5k - P1' ? ' selected' : ''}>GFT $5k - P1</option><option${t.account === 'GFT $10k - P1' ? ' selected' : ''}>GFT $10k - P1</option><option${t.account === 'GFT $25k - P1' ? ' selected' : ''}>GFT $25k - P1</option><option${t.account === 'GFT $50k - P1' ? ' selected' : ''}>GFT $50k - P1</option><option${t.account === 'GFT $5k - P2' ? ' selected' : ''}>GFT $5k - P2</option><option${t.account === 'GFT $10k - P2' ? ' selected' : ''}>GFT $10k - P2</option><option value="__custom__">＋ Custom…</option></select><input type="text" class="form-input" id="e-acc-custom" placeholder="Enter account name…" style="display:none;margin-top:6px" value="${['PaperTrading','GFT $5k - P1','GFT $10k - P1','GFT $25k - P1','GFT $50k - P1','GFT $5k - P2','GFT $10k - P2'].includes(t.account) ? '' : t.account}"></div>
      <div class="form-field"><label class="form-label">Strategy</label><select class="form-select" id="e-strat"><option${t.strategy === 'NxtGen - Mod' ? ' selected' : ''}>NxtGen - Mod</option><option${t.strategy === 'IRL > ERL' ? ' selected' : ''}>IRL > ERL</option><option${t.strategy === 'ERL > IRL' ? ' selected' : ''}>ERL > IRL</option></select></div>
      <div class="form-field"><label class="form-label">TF Alignment</label><select class="form-select" id="e-tf" onchange="_handleCustomSelect(this,'e-tf-custom')"><option${t.tf === '30m > 3m' ? ' selected' : ''}>30m > 3m</option><option${t.tf === '1h > 5m' ? ' selected' : ''}>1h > 5m</option><option${t.tf === '1h > 3m' ? ' selected' : ''}>1h > 3m</option><option${t.tf === '4h > 15m' ? ' selected' : ''}>4h > 15m</option><option${t.tf === '4h > 5m' ? ' selected' : ''}>4h > 5m</option><option${t.tf === '4h > 3m' ? ' selected' : ''}>4h > 3m</option><option${t.tf === '15m > 3m' ? ' selected' : ''}>15m > 3m</option><option value="__custom__">＋ Custom…</option></select><input type="text" class="form-input" id="e-tf-custom" placeholder="e.g. 2h > 5m" style="display:none;margin-top:6px" value="${['30m > 3m','1h > 5m','1h > 3m','4h > 15m','4h > 5m','4h > 3m','15m > 3m'].includes(t.tf) ? '' : t.tf}"></div>
    </div>
...
function buildEntryForm() {
  document.getElementById('entry-form-content').innerHTML = `
    <div class="form-grid">
      ...
      <div class="form-field"><label class="form-label">Strategy</label><select class="form-select"><option>NxtGen - Mod</option><option>IRL > ERL</option><option>ERL > IRL</option></select></div>
      ...
      <div class="form-field"><label class="form-label">Account</label><select class="form-select"><option>PaperTrading</option><option>GFT $5k - P1</option><option>GFT $10k - P1</option><option>GFT $25k - P1</option><option>GFT $50k - P1</option><option>GFT $5k - P2</option><option>GFT $10k - P2</option><option value="__custom__">＋ Custom…</option></select></div>
      <div class="form-field"><label class="form-label">Rating</label><select class="form-select"><option>★★★★★  (5)</option><option>★★★★☆  (4)</option><option>★★★☆☆  (3)</option></select></div>
...
function setStarRating(id, n) {
  n = Math.max(3, Math.min(5, n)); // clamp 3–5
  const editor = document.getElementById('star-editor');
  if (!editor) return;
  editor.dataset.rating = n;
  // Stars are now rendered as [3,4,5] so index 0=3★, 1=4★, 2=5★
  editor.querySelectorAll('span').forEach((s, i) => {
    const starN = i + 3;
    s.textContent = starN <= n ? '★' : '☆';
    s.className   = starN <= n ? 'lit' : '';
  });
}

// Show/hide a custom text input when "＋ Custom…" is selected
...
async function _saveEdit(id) {
  ...
  const accSelVal = get('e-acc'), stratVal = get('e-strat'), tfSelVal = get('e-tf');
  // Resolve custom values if "＋ Custom…" was chosen
  const accVal = accSelVal === '__custom__' ? (get('e-acc-custom') || '').trim() || accSelVal : accSelVal;
  const tfVal  = tfSelVal  === '__custom__' ? (get('e-tf-custom')  || '').trim() || tfSelVal  : tfSelVal;
  const editor = document.getElementById('star-editor');
...
// ══════════════════════════════════════════════════════
// SHARE MODAL — NxTGen Trade Card v2
// Sharp-edged, ultra-high-res, % / USD / both toggle
// ══════════════════════════════════════════════════════

let _shareFmt      = 'jpg';   // 'jpg' | 'pdf'
let _sharePnlMode  = 'pct';   // 'pct' | 'usd' | 'both'
let _shareCardTheme = 'dark'; // 'dark' | 'light'
let _shareTradeId  = null;
  ...
  <div class="sm-modal" id="sm-modal">
    ...
    <div class="sm-header">
      ...
      <div class="sm-header-right">
        <button class="sm-icon-btn" id="sm-theme-btn" onclick="smToggleTheme()" title="Toggle card theme">
          ${_shareCardTheme === 'dark' ? '☀️' : '🌙'}
        </button>
    ...
    <div class="sm-body">
      ...
      <div class="sm-card-wrap">
        <div class="sm-card ${_shareCardTheme}" id="sm-card">
          ...
          <svg class="sm-chart-bg" viewBox="0 0 600 260" preserveAspectRatio="none" aria-hidden="true">
            ...
            <polyline id="sm-chartline"
              points="0,220 70,175 140,195 210,120 280,140 350,75 420,95 490,38 600,60"
              fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/>
            <polygon id="sm-chartfill"
              points="0,220 70,175 140,195 210,120 280,140 350,75 420,95 490,38 600,60 600,260 0,260"
              fill="url(#chartGrad)"/>
          </svg>
          ...
          <div class="sm-top-bar">
            <div class="sm-brand">
              ...
              <div class="sm-brand-text">
                <span class="sm-brand-name">NxTGen</span>
                <span class="sm-brand-sub">Trading Journal</span>
              </div>
            </div>
          ...
          <div class="sm-hero">
            <div class="sm-hero-left">
              ...
              <div class="sm-direction" id="sm-direction">
                <span class="sm-dir-arrow" id="sm-dir-arrow"></span>
                <span id="sm-dir-text">${t.pos} &nbsp;·&nbsp; ${t.kz}</span>
              </div>
            </div>
            <div class="sm-hero-right">
              <div class="sm-pnl-block" id="sm-pnl-block"></div>
              <div class="sm-pnl-label">Net PnL</div>
            </div>
          </div>
          ...
          <div class="sm-stats">
            <div class="sm-stat">
              <div class="sm-stat-lbl">RISK · REWARD</div>
              <div class="sm-stat-val" id="sm-rr">${t.rr}</div>
            </div>
            <div class="sm-stat sm-stat-center">
              <div class="sm-stat-lbl">TF ALIGNMENT</div>
              <div class="sm-stat-val" id="sm-tf">${t.tf || '—'}</div>
            </div>
            <div class="sm-stat">
              <div class="sm-stat-lbl">STRATEGY</div>
              <div class="sm-stat-val sm-stat-val-sm" id="sm-strategy">${t.strategy || '—'}</div>
            </div>
          </div>
          ...
          <!-- NOTES -->
          <div class="sm-notes-row" id="sm-notes-row">
            <div class="sm-notes" id="sm-notes"></div>
          </div>

          <!-- BOTTOM ROW -->
          <div class="sm-bottom-row">
            <div class="sm-stars" id="sm-stars"></div>
            <div class="sm-risk-tag" id="sm-risk-tag">${t.risk || '—'} risk</div>
          </div>

          <!-- FOOTER -->
          <div class="sm-footer">
            <span class="sm-footer-url">dabossmira.github.io/NxTGen-Journal</span>
            <span class="sm-footer-brand">⚡ NxTGen</span>
          </div>
      ...
      <div class="sm-section">
        ...
        <div class="sm-usd-row" id="sm-usd-row" style="display:none">
          ...
          <div class="sm-usd-input-wrap">
            <span class="sm-usd-prefix">$</span>
            <input class="sm-usd-input" id="sm-acct-size" type="number" value="5000" min="1" oninput="smRefreshPnl()">
          </div>
        </div>
      </div>
      ...
      <div class="sm-section">
        ...
        <div class="sm-fmt-row">
          <button class="sm-fmt-btn active" id="sm-fmt-jpg" onclick="smSetFmt('jpg',this)">
            <span class="sm-fmt-icon">🖼</span>
            <div>
              <div class="sm-fmt-name">JPG Image</div>
              <div class="sm-fmt-desc">Best for social sharing</div>
            </div>
          </button>
          <button class="sm-fmt-btn" id="sm-fmt-pdf" onclick="smSetFmt('pdf',this)">
            <span class="sm-fmt-icon">📄</span>
            <div>
              <div class="sm-fmt-name">PDF Document</div>
              <div class="sm-fmt-desc">Best for records &amp; print</div>
            </div>
          </button>
        </div>
      </div>
      ...
      <div class="sm-actions">
        <button class="sm-btn sm-btn-primary" id="sm-dl-btn" onclick="smDownload()">
          <span class="sm-spinner" id="sm-dl-spin"></span>
          <span id="sm-dl-lbl">⬇ Download</span>
        </button>
        <button class="sm-btn sm-btn-ghost" id="sm-share-btn" onclick="smShareNative()">
          <span class="sm-spinner sm-spinner-light" id="sm-share-spin"></span>
          <span id="sm-share-lbl">🔗 Share</span>
        </button>
      </div>
...
function smPopulateCard(id) {
  ...
  // Chart line color
  const chartLine = document.getElementById('sm-chartline');
  const chartFill = document.getElementById('sm-chartfill');
  const chartColor = t.outcome === 'Win' ? '#34d399' : t.outcome === 'Loss' ? '#f87171' : '#fbbf24';
  if (chartLine) { chartLine.style.stroke = chartColor; }
  if (chartFill) {
    // Update gradient stop
    const grad = document.getElementById('chartGrad');
    if (grad) { grad.querySelector('stop').style.stopColor = chartColor; }
  }

  // Stars
  const stars = document.getElementById('sm-stars');
  if (stars) stars.innerHTML = [1,2,3,4,5].map(n => `<span class="${n <= t.rating ? 'sm-star-lit' : 'sm-star-dim'}">★</span>`).join('');

  // Notes
  const notesRow = document.getElementById('sm-notes-row');
  const notesEl  = document.getElementById('sm-notes');
  if (notesEl && t.notes && t.notes.trim()) {
    notesEl.textContent = t.notes.length > 130 ? t.notes.substring(0,127)+'…' : t.notes;
...
function smRefreshPnl() {
  const id = _shareTradeId;
  const t = trades.find(x => x.id === id);
  if (!t) return;

  const pnlBlock = document.getElementById('sm-pnl-block');
  if (!pnlBlock) return;

  const sign   = t.pnl >= 0 ? '+' : '';
  const pctStr = sign + t.pnl + '%';

  let usdStr = '';
  const acctSizeEl = document.getElementById('sm-acct-size');
  const acctSize   = acctSizeEl ? parseFloat(acctSizeEl.value) || 5000 : 5000;
  const usdVal     = (t.pnl / 100) * acctSize;
  const usdSign    = usdVal >= 0 ? '+' : '';
  usdStr           = usdSign + '$' + Math.abs(usdVal).toLocaleString('en-US', {minimumFractionDigits:2, maximumFractionDigits:2});

  const cls = t.pnl > 0 ? 'pnl-win' : t.pnl < 0 ? 'pnl-loss' : 'pnl-be';

  if (_sharePnlMode === 'pct') {
    pnlBlock.innerHTML = `<span class="sm-pnl-main ${cls}">${pctStr}</span>`;
  } else if (_sharePnlMode === 'usd') {
    pnlBlock.innerHTML = `<span class="sm-pnl-main ${cls}">${usdStr}</span>`;
  } else {
    pnlBlock.innerHTML = `<span class="sm-pnl-main ${cls}">${pctStr}</span><span class="sm-pnl-sub ${cls}">${usdStr}</span>`;
  }
}

// ── Controls ───────────────────────────────────────────
function smSetPnlMode(mode, btn) {
  _sharePnlMode = mode;
  document.querySelectorAll('#sm-pnl-seg .sm-seg-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  const usdRow = document.getElementById('sm-usd-row');
  if (usdRow) usdRow.style.display = (mode === 'pct') ? 'none' : '';
  smRefreshPnl();
}

function smSetFmt(fmt, btn) {
  _shareFmt = fmt;
  document.querySelectorAll('.sm-fmt-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
}

function smToggleTheme() {
  _shareCardTheme = _shareCardTheme === 'dark' ? 'light' : 'dark';
  const card = document.getElementById('sm-card');
  if (card) { card.className = 'sm-card ' + _shareCardTheme; }
  const btn = document.getElementById('sm-theme-btn');
  if (btn) btn.textContent = _shareCardTheme === 'dark' ? '☀️' : '🌙';
}
...
// ── Export helpers ─────────────────────────────────────
function _smSetLoading(btnId, spinId, lblId, on, lbl) {
  ...
  if (lb) { lb.style.opacity = on ? '0' : '1'; if (lbl && !on) lb.textContent = lbl; }
}

async function _smCapture() {
  const card = document.getElementById('sm-card');
  return await html2canvas(card, {
    scale: 5,            // 5× → very high resolution
    useCORS: true,
...
async function smDownload() {
  ...
  try {
    const t        = trades.find(x => x.id === _shareTradeId);
    const filename = `NxTGen_${(t||{pair:'Trade'}).pair}_${(t||{date:'trade'}).date}`;
    const canvas   = await _smCapture();
    ...
    if (_shareFmt === 'pdf') {
      ...
      pdf.addImage(canvas.toDataURL('image/jpeg', 0.96), 'JPEG', 15, 20, iw, ih);
    ...
    console.error('smDownload error:', e);
    showToast('Export failed — try again', 'danger');
  }
...
async function smShareNative() {
  ...
  try {
    ...
    canvas.toBlob(async blob => {
      const file = new File([blob], `NxTGen_${(t||{pair:'Trade'}).pair}.jpg`, { type: 'image/jpeg' });
      const shareData = {
        files: [file],
        title: `NxTGen Trade · ${(t||{}).pair} · ${(t||{}).outcome}`,
        text:  `${(t||{}).pair} ${(t||{}).pos} · ${(t||{}).pnl >= 0 ? '+' : ''}${(t||{}).pnl}% — NxTGen Trading Journal`,
      };
      if (navigator.canShare && navigator.canShare(shareData)) {
        try { await navigator.share(shareData); showToast('Shared! 🔗','success'); }
        catch(e) { if (e.name !== 'AbortError') _smClipboardFallback(canvas); }
      } else { _smClipboardFallback(canvas); }
      _smSetLoading('sm-share-btn','sm-share-spin','sm-share-lbl', false, '🔗 Share');
    }, 'image/jpeg', 0.96);
  } catch(e) {
    console.error('smShareNative error:', e);
    showToast('Share failed — use Download instead','danger');
    _smSetLoading('sm-share-btn','sm-share-spin','sm-share-lbl', false, '🔗 Share');
  }
}

function _smClipboardFallback(canvas) {
  canvas.toBlob(async blob => {
    try {
      ...
      const link = document.createElement('a');
      link.download = 'NxTGen_Trade.jpg';
      link.href = canvas.toDataURL('image/jpeg', 0.96);
      link.click();
      showToast('Saved as image! 🖼','success');
    }
  }, 'image/png');
}

function _smEnsureLibs(cb) {
  ...
  h2c.onload = () => {
    const jpdf = document.createElement('script');
    jpdf.src = 'https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js';
    jpdf.onload = cb;
    document.head.appendChild(jpdf);
  };
