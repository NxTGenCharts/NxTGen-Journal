...
// ── CUSTOM ACCOUNTS list (persisted in localStorage) ─────────────────────
...
function _saveCustomAccounts(list) {
  try { localStorage.setItem('nxtgen_custom_accounts', JSON.stringify(list)); } catch {}
}
function _buildAccountOptions(current) {
  const list = _getCustomAccounts();
  const opts = list.map(a => `<option${a === current ? ' selected' : ''}>${a}</option>`).join('');
  return opts;
}
function _openManageAccounts() {
  ...
  <div class="acc-manager-modal">
    ...
    <div class="acc-manager-body">
      <div id="acc-mgr-list" class="acc-mgr-list">
        ${list.map((a, i) => `
          <div class="acc-mgr-item" id="acc-mgr-item-${i}">
            ...
            <div class="acc-mgr-actions">
              ...
              ${i < 2 ? '<button class="acc-mgr-btn del" disabled title="Default account">🗑</button>' : `<button onclick="_deleteAccount(${i})" class="acc-mgr-btn del">🗑</button>`}
            </div>
          </div>`).join('')}
      </div>
...
function _addAccount() {
  const inp = document.getElementById('acc-mgr-input');
  if (!inp) return;
  const name = inp.value.trim();
  if (!name) return;
  const list = _getCustomAccounts();
  if (list.includes(name)) { showToast('Account already exists', 'danger'); return; }
  list.push(name);
  _saveCustomAccounts(list);
  inp.value = '';
  _rebuildAccMgrList();
  _refreshAccountDropdown();
  showToast('Account added ✓', 'restore');
}
function _deleteAccount(i) {
  const list = _getCustomAccounts();
  if (i < 2) return; // protect defaults
  const name = list[i];
  list.splice(i, 1);
  _saveCustomAccounts(list);
  _rebuildAccMgrList();
  _refreshAccountDropdown();
  showToast(name + ' removed', 'restore');
}
function _editAccount(i) {
  const list = _getCustomAccounts();
  const item = document.getElementById('acc-mgr-item-' + i);
  if (!item) return;
  const current = list[i];
  item.innerHTML = `
    <input type="text" class="acc-mgr-input" id="acc-edit-${i}" value="${current}" style="flex:1;margin-right:8px" onkeydown="if(event.key==='Enter')_saveEditAccount(${i})">
    <div class="acc-mgr-actions">
...
function _saveEditAccount(i) {
  ...
  _rebuildAccMgrList();
  _refreshAccountDropdown();
  showToast('Account updated ✓', 'restore');
}
...
async function _saveEdit(id) {
  ...
  const accVal = get('e-acc'), stratVal = get('e-strat'), tfVal = get('e-tf') === '__custom__' ? (get('e-tf-custom')||'').trim() || get('e-tf') : get('e-tf');
  const editor = document.getElementById('star-editor');
